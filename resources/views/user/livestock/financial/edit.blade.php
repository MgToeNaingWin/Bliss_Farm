{{-- resources/views/user/livestock/financial/edit.blade.php --}}
    ...
    <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white rounded-lg shadow overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200">
                <h2 class="text-xl font-bold text-gray-800">ငွေပေးချေမှုပြင်ဆင်ရန်</h2>
            </div>

            <form method="POST" action="{{ route('user.financial.update', $record) }}" class="p-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="type" id="type" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            ...
                            <option value="income" {{ old('type', $record->type) == 'income' ? 'selected' : '' }}>ဝင်ငွေ</option>
                            <option value="expense" {{ old('type', $record->type) == 'expense' ? 'selected' : '' }}>ထွက်ငွေ</option>
                        </select>
                        @error('type')
                        ...
                        <select name="category" id="category" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            ...
                            <optgroup label="ဝင်ငွေ">
                                @foreach($incomeCategories as $category)
                                    <option value="{{ $category }}" class="income-category" {{ old('category', $record->category) == $category ? 'selected' : '' }}>
                                        {{ $category }}
                                    </option>
                                @endforeach
                            </optgroup>
                            <optgroup label="ထွက်ငွေ">
                                @foreach($expenseCategories as $category)
                                    <option value="{{ $category }}" class="expense-category" {{ old('category', $record->category) == $category ? 'selected' : '' }}>
                                        {{ $category }}
                                    </option>
                                @endforeach
                            </optgroup>
                    ...
                    <div>
                        ...
                        <input type="text" name="sub_category" value="{{ old('sub_category', $record->sub_category) }}"
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" name="amount" value="{{ old('amount', $record->amount) }}" required
                               class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                        @error('amount')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="date" name="transaction_date" value="{{ old('transaction_date', $record->transaction_date->format('Y-m-d')) }}" required
                               class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                    ...
                    <div>
                        ...
                        <select name="payment_method" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            ...
                            @foreach($paymentMethods as $method)
                                <option value="{{ $method }}" {{ old('payment_method', $record->payment_method) == $method ? 'selected' : '' }}>
                                    {{ $method }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="livestock_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            ...
                            @foreach($livestocks as $id => $name)
                                <option value="{{ $id }}" {{ old('livestock_id', $record->livestock_id) == $id ? 'selected' : '' }}>
                                    {{ $name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="status" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            <option value="completed" {{ old('status', $record->status) == 'completed' ? 'selected' : '' }}>ပြီးစီး</option>
                            <option value="pending" {{ old('status', $record->status) == 'pending' ? 'selected' : '' }}>ဆောင်ရွက်ဆဲ</option>
                            <option value="cancelled" {{ old('status', $record->status) == 'cancelled' ? 'selected' : '' }}>ပယ်ဖျက်</option>
                        </select>
                        @error('status')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="description" value="{{ old('description', $record->description) }}" required
                               class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                        @error('description')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="notes" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('notes', $record->notes) }}</textarea>
                        @error('notes')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
