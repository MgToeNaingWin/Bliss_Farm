...
<div class="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-8 pt-6 pb-16">
    ...
    <!-- Back Link -->
    <div class="mb-5">
        <a href="{{ route('newsIndex') }}" class="inline-flex items-center gap-1.5 text-sm text-stone-500 hover:text-emerald-700 transition font-medium">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <article class="lg:col-span-8">
            {{-- Hero Image --}}
            @php $imageName = $news->{'news-img'} ?? null; @endphp
            <div class="reveal rounded-xl overflow-hidden bg-stone-100 mb-6">
                @if($imageName && file_exists(public_path('newsImage/' . $imageName)))
                    <img src="{{ asset('newsImage/' . $imageName) }}" alt="{{ $news->title }}" class="w-full h-64 sm:h-80 lg:h-[420px] object-cover">
                @else
            ...
            <div class="reveal flex flex-wrap items-center gap-2 mb-4">
                <span class="bg-stone-900 text-white text-[11px] font-bold px-2.5 py-1 rounded">သတင်း</span>
                <span class="text-stone-400 text-xs">{{ $news->created_at ? $news->created_at->format('MMMM d, Y') : '' }}</span>
                <span class="text-stone-300">•</span>
                <span class="text-stone-400 text-xs flex items-center gap-1"><i class="fa-solid fa-eye text-[10px]"></i> {{ number_format($news->view_count ?? 0) }}</span>
                <span class="text-stone-300">•</span>
                <span class="text-stone-400 text-xs flex items-center gap-1"><i class="fa-regular fa-clock text-[10px]"></i> {{ ceil(str_word_count($news->description) / 200) }} min read</span>
            </div>
            ...
            {{-- Writer --}}
            <div class="reveal flex items-center gap-3 mb-6 pb-6 border-b border-stone-100">
                <div class="w-10 h-10 bg-emerald-100 rounded-full flex items-center justify-center">
                    <span class="text-emerald-700 text-sm font-bold">{{ strtoupper(substr($news->writer ?? 'T', 0, 1)) }}</span>
                </div>
                <div>
                    <p class="text-sm font-semibold text-stone-800">{{ $news->writer ?? 'Thukha Farm' }}</p>
                    <p class="text-[11px] text-stone-400">စာတည်းနေရာ</p>
                </div>
            </div>

            {{-- Body --}}
            <div class="reveal text-stone-700 leading-[1.85] space-y-4 mb-8">
                @php $paragraphs = array_filter(explode("\n", $news->description)); @endphp
            ...
            <div class="reveal flex flex-wrap items-center gap-2 mb-8 pb-6 border-b border-stone-100" id="reactions-container">
                ...
                <div class="ml-auto flex items-center gap-2">
                    ...
                    <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(url('/news/' . $news->id)) }}" target="_blank" class="w-8 h-8 bg-stone-100 hover:bg-blue-100 text-stone-400 hover:text-blue-600 rounded-lg flex items-center justify-center transition text-sm"><i class="fa-brands fa-facebook-f"></i></a>
                    <a href="https://twitter.com/intent/tweet?url={{ urlencode(url('/news/' . $news->id)) }}&text={{ urlencode($news->title) }}" target="_blank" class="w-8 h-8 bg-stone-100 hover:bg-sky-100 text-stone-400 hover:text-sky-500 rounded-lg flex items-center justify-center transition text-sm"><i class="fa-brands fa-twitter"></i></a>
            ...
            <div class="reveal">
                ...
                <div class="space-y-3" id="comments-list">
                    @forelse($comments->filter(fn($c) => !$c->parent_id) as $comment)
                        @include('user.news._comment', ['comment' => $comment])
                    @empty
        ...
        <aside class="lg:col-span-4 space-y-6">
            ...
            <div class="bg-white border border-stone-100 rounded-xl p-5">
                <h3 class="font-[Bricolage_Grotesque] text-sm font-bold text-stone-900 mb-4 flex items-center gap-2">
                    <span class="w-1 h-4 bg-stone-900 rounded-full"></span> Latest News
                </h3>
                <div class="space-y-4">
                    @forelse($latestNews as $item)
                        @php $img = $item->{'news-img'} ?? null; @endphp
                        <a href="{{ route('newsShow', $item->id) }}" class="sidebar-link flex gap-3 group">
                            <div class="w-20 h-16 rounded-lg overflow-hidden bg-stone-100 flex-shrink-0">
                                @if($img && file_exists(public_path('newsImage/' . $img)))
                                    <img src="{{ asset('newsImage/' . $img) }}" class="w-full h-full object-cover" alt="">
                                @else
                            ...
                            <div class="min-w-0 flex-1">
                                <p class="sidebar-title text-[13px] font-bold text-stone-800 leading-snug line-clamp-2 transition-colors">{{ $item->title }}</p>
                                <p class="text-[10px] text-stone-400 mt-1">{{ $item->created_at ? $item->created_at->format('MMM d, Y') : '' }}</p>
                            </div>
                        </a>
                    @empty
                        <p class="text-xs text-stone-400 text-center py-4">သတင်းများ မရှိသေးပါ။</p>
                    @endforelse
                </div>
                <a href="{{ route('newsIndex') }}" class="block text-center text-xs font-semibold text-emerald-700 hover:text-emerald-800 mt-4 pt-3 border-t border-stone-100 transition">
            ...
            <div class="bg-white border border-stone-100 rounded-xl p-5">
                <h3 class="font-[Bricolage_Grotesque] text-sm font-bold text-stone-900 mb-4 flex items-center gap-2">
                    <span class="w-1 h-4 bg-amber-500 rounded-full"></span> Most Read
                </h3>
                <div class="space-y-4">
                    @forelse($mostRead as $index => $item)
                        @php $img = $item->{'news-img'} ?? null; @endphp
                        <a href="{{ route('newsShow', $item->id) }}" class="sidebar-link flex gap-3 group">
                            ...
                            @if($img && file_exists(public_path('newsImage/' . $img)))
                                <img src="{{ asset('newsImage/' . $img) }}" class="w-14 h-14 rounded-lg object-cover flex-shrink-0" alt="">
                            @endif
                        </a>
                    @empty
                        <p class="text-xs text-stone-400 text-center py-4">သတင်းများ မရှိသေးပါ။</p>
...
function toggleReaction(type) {
    fetch('{{ route("newsReaction.toggle", $news->id) }}', {
        method: 'POST',
...
function submitComment() {
    ...
    if (!body) return;
    fetch('{{ route("newsComment.store", $news->id) }}', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}', 'Accept': 'application/json' },
